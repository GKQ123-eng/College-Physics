...
class ThermoelectricCalculator:
            ...
            if data is not None:
                try:
                    ...
                    if columns:
                        ...
                        print(f"电阻率范围: {min(resistivity*1e5)}-{max(resistivity*1e5)} μΩ·m")
    ...
    def create_interpolators(self, material_type, composition):
        ...
        try:
            ...
            print(f"电阻率范围: {min(resistivity*1e5):.2f}-{max(resistivity*1e5):.2f} μΩ·m")
    ...
    def calculate_efficiency(self, Th, Tc, material_type, composition, current_density, x=None, T=None):
        ...
        η_λ = ∫[0→L] J_λ·S_λ(x)·dT/dx·dx - ∫[0→L] J²_λ·ρ_λ(x)·dx
              -----------------------------------------------
                          ∫[0→L] q_λ(x)·dx
        ...
        try:
            ...
            dTdx[1:-1] = (T[2:] - T[:-2]) / (2*dx)  # 中心节点
            dTdx[0] = (T[1] - T[0]) / dx            # 首节点(前向差分)
            dTdx[-1] = (T[-1] - T[-2]) / dx         # 末节点(后向差分)
            ...
            for i in range(n_points):
                T_safe = np.clip(T[i], 300, 700)  # 限制在有效范围内
                seebeck[i] = self.interpolators[interp_key]["seebeck"](T_safe)
                resistivity[i] = self.interpolators[interp_key]["resistivity"](T_safe)
                thermal_cond[i] = self.interpolators[interp_key]["thermal_cond"](T_safe)
            ...
            # 计算第一项积分: ∫[0→L] J·S(x)·dT/dx·dx (塞贝克功率)
            seebeck_power_integrand = J * seebeck * dTdx
            seebeck_power = np.trapz(seebeck_power_integrand, x)
            
            # 计算第二项积分: ∫[0→L] J²·ρ(x)·dx (焦耳热损失)
            joule_heat_integrand = J**2 * resistivity
            joule_heat = np.trapz(joule_heat_integrand, x)
            
            # 计算分母积分: ∫[0→L] q(x)·dx (热流输入)
            # 热流密度应该是从高温端流向低温端，确保正值
            if np.mean(q) < 0:
                q = -q  # 反转热流方向
            
            heat_input = np.trapz(q, x)
            
            # 计算净功率输出
            net_power = seebeck_power - joule_heat
            
            # 计算效率
            if heat_input > 0 and net_power > 0:
                efficiency = net_power / heat_input
                
                # 验证效率不超过卡诺效率
                carnot_eff = (Th - Tc) / Th
                if efficiency > carnot_eff:
                    print(f"警告: 计算效率 {efficiency*100:.2f}% 超过卡诺效率 {carnot_eff*100:.2f}%")
                    efficiency = carnot_eff * 0.9  # 限制在卡诺效率的90%以内
            else:
                efficiency = 0.0
            
            return efficiency * 100, net_power  # 返回百分比效率和净功率密度
...
class ThermoelectricApp(QMainWindow):
    ...
    def update_temperature_plots(self, x_p, T_p, x_n, T_n):
        ...
        try:
            ...
            print(f"P型格点位置: {grid_points_p}")
            print(f"P型温度数据: {T_p}")
            print(f"N型格点数量: {n_points_n}")
            print(f"N型格点位置: {grid_points_n}")
            print(f"N型温度数据: {T_n}")
            
            # 绘制新数据 - 使用星号标记和细线
            ax1.plot(grid_points_p, T_p, 'b*-', markersize=5)  # P型用蓝色星号
            ax2.plot(grid_points_n, T_n, 'r*-', markersize=5)  # N型用红色星号
            ...
            for ax in [ax1, ax2]:
                ax.set_xlabel("格点位置")
                ax.set_ylabel("温度 (K)")
                ax.set_xlim(0.5, 10.5)  # 稍微扩展范围以显示完整的标记
                ax.set_ylim(min_temp - 10, max_temp + 10)
                ax.set_xticks(range(1, 11))  # 设置整数刻度
                ax.grid(True)
    ...
    def update_efficiency_plots(self):
        ...
        try:
            ...
            # 创建与界面选项一致的电流密度范围
            p_currents = np.linspace(-2.0, -0.5, 16)  # P型电流密度范围
            n_currents = np.linspace(25, 40, 16)     # N型电流密度范围
            ...
            # 绘制P型效率曲线 - 简化风格
            ax1.plot(p_currents, p_efficiencies, 'b-o', linewidth=1.5, markersize=4)
            ax1.set_title("P型材料效率")
            ax1.set_xlabel("电流密度 (A/cm²)")
            ax1.set_ylabel("效率 (%)")
            ax1.set_xlim(-2.0, -0.5)
            ax1.set_ylim(0, max(max(p_efficiencies)*1.1, 0.1))
            ax1.grid(True)
            
            # 绘制N型效率曲线 - 简化风格
            ax2.plot(n_currents, n_efficiencies, 'r-s', linewidth=1.5, markersize=4)
            ax2.set_title("N型材料效率")
            ax2.set_xlabel("电流密度 (A/cm²)")
            ax2.set_ylabel("效率 (%)")
            ax2.set_xlim(25, 40)
            ax2.set_ylim(0, max(max(n_efficiencies)*1.1, 0.1))
            ax2.grid(True)
