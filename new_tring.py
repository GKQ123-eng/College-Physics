...
class ThermoelectricApp(QMainWindow):
    ...
    def update_efficiency_plots(self):
        ...
        try:
            ...
            # 创建更合适的电流密度范围
            p_currents = np.linspace(-5.0, -0.1, 20)  # P型电流密度范围（扩大范围）
            n_currents = np.linspace(5.0, 45.0, 20)    # N型电流密度范围（扩大范围）
            ...
            if p_efficiencies:
                ...
                max_p_j = p_currents[max_p_eff_idx]
                print(f"P型最大效率: {max_p_eff:.2f}% 在电流密度 {max_p_j:.2f}A/cm²")
            
            if n_efficiencies:
                max_n_eff_idx = np.argmax(n_efficiencies)
                max_n_eff = n_efficiencies[max_n_eff_idx]
                max_n_j = n_currents[max_n_eff_idx]
                print(f"N型最大效率: {max_n_eff:.2f}% 在电流密度 {max_n_j:.2f}A/cm²")
            
            # 绘制P型效率曲线
            ax1.plot(p_currents, p_efficiencies, 'b-', linewidth=1.5)
            ax1.scatter(p_currents, p_efficiencies, color='blue', s=20, marker='o')
            ax1.scatter(current_p, p_current_eff, color='red', s=80, marker='*', 
                       label=f'当前: {current_p}A/cm², {p_current_eff:.2f}%')
            
            if p_efficiencies:
                ax1.scatter(max_p_j, max_p_eff, color='green', s=80, marker='s',
                           label=f'最大: {max_p_j:.2f}A/cm², {max_p_eff:.2f}%')
            
            ax1.set_title("P型材料效率")
            ax1.set_xlabel("电流密度 (A/cm²)")
            ax1.set_ylabel("效率 (%)")
            # 设置P型横坐标范围，稍微扩大以显示完整
            p_min = min(p_currents) * 1.1
            p_max = max(p_currents) * 0.5 if max(p_currents) < 0 else max(p_currents) * 1.1
            ax1.set_xlim(p_min, p_max)
            
            # 设置P型纵坐标范围
            if p_efficiencies:
                y_max = max(p_efficiencies) * 1.2
                ax1.set_ylim(0, max(y_max, 5.0))  # 最小上限为5%
            else:
                ax1.set_ylim(0, 5.0)
            ...
            # 绘制N型效率曲线
            ax2.plot(n_currents, n_efficiencies, 'r-', linewidth=1.5)
            ax2.scatter(n_currents, n_efficiencies, color='red', s=20, marker='o')
            ax2.scatter(current_n, n_current_eff, color='blue', s=80, marker='*',
                       label=f'当前: {current_n}A/cm², {n_current_eff:.2f}%')
            
            if n_efficiencies:
                ax2.scatter(max_n_j, max_n_eff, color='green', s=80, marker='s',
                           label=f'最大: {max_n_j:.2f}A/cm², {max_n_eff:.2f}%')
            
            ax2.set_title("N型材料效率")
            ax2.set_xlabel("电流密度 (A/cm²)")
            ax2.set_ylabel("效率 (%)")
            # 设置N型横坐标范围，稍微扩大以显示完整
            n_min = min(n_currents) * 0.9
            n_max = max(n_currents) * 1.1
            ax2.set_xlim(n_min, n_max)
            
            # 设置N型纵坐标范围
            if n_efficiencies:
                y_max = max(n_efficiencies) * 1.2
                ax2.set_ylim(0, max(y_max, 5.0))  # 最小上限为5%
            else:
                ax2.set_ylim(0, 5.0)
